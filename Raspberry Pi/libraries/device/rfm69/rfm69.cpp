...
unsigned char RFM69::initialize(unsigned char freqBand, unsigned short ID, unsigned char networkID, unsigned char intPin, unsigned char rstPin, unsigned char spiBus)
{
    ...
    GPIO rstGPIO(rstPin);
    rstGPIO.setDirection(OUTPUT);
    rstGPIO.setValue(HIGH);
    COSMOS_SLEEP(0.1);
    //digitalWrite(rstPin, LOW);
    rstGPIO.setValue(LOW);
    COSMOS_SLEEP(0.1);
    //wiringPiSPISetup(spiBus, 1000000);
    this->spi = new SPIDevice(spiBus, 0);
    this->spi->setSpeed(1000000);
    ...
    //Verify chip syncing
    unsigned int start = currentmjd();
    unsigned char timeout = 50;

    do writeReg(REG_SYNCVALUE1, 0xAA); while (readReg(REG_SYNCVALUE1) != 0xaa && currentmjd()-start < timeout);
    start = currentmjd();
    do writeReg(REG_SYNCVALUE1, 0x55); while (readReg(REG_SYNCVALUE1) != 0x55 && currentmjd()-start < timeout);
    ...
    start = currentmjd();
    while (((readReg(REG_IRQFLAGS1) & RF_IRQFLAGS1_MODEREADY) == 0x00) && currentmjd()-start < timeout); // wait for ModeReady
...
void RFM69::send(unsigned short toAddress, const void* buffer, unsigned char bufferSize, bool requestACK)
{
    ...
    unsigned int now = currentmjd();
    while (!canSend() && currentmjd() - now < RF69_CSMA_LIMIT_MS) receiveDone();
...
bool RFM69::sendWithRetry(unsigned short toAddress, const void* buffer, unsigned char bufferSize, unsigned char retries, unsigned char retryWaitTime)
{
    ...
    for (i = 0; i <= retries; i++)
    {
        ...
        sentTime = currentmjd();
        while (currentmjd() - sentTime < retryWaitTime)
...
void RFM69::sendFrame(unsigned short toAddress, const void* buffer, unsigned char bufferSize, bool requestACK, bool sendACK)
{
    ...
    this->spi->write(data, bufferSize+5);
    readReg(REG_IRQFLAGS2);
    // no need to wait for transmit mode to be ready since its handled by the radio
    setMode(RF69_MODE_TX);
    while ((readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PACKETSENT) == 0x00) COSMOS_SLEEP(0.01); // wait for PacketSent
    _haveData = false;
}
...
void RFM69::interruptHandler()
{
    if (_mode == RF69_MODE_RX && (readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY))
    {
        ...
        this->spi->write(data, PAYLOADLEN+1);
        TARGETID = data[1];
        SENDERID = data[2];
...
unsigned char RFM69::readReg(unsigned char addr)
{
    ...
    data[1] = this->spi->readRegister(data[0]);
    return data[1];
}
void RFM69::writeReg(unsigned char addr, unsigned char val)
{
    ...
    data[1]=val;
    //wiringPiSPIDataRW(0, data, 2);
    this->spi->writeRegister(data[0], data[1]);
}
void RFM69::isr0()
{
